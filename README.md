...
Este código é o `CarrinhoService.java` 
onde nele tem as funções de adicionar, 
atualizar e remover os itens dentro do carrinho do usuário. 
Também tem as funções de deletar, finalizar e cancelar o carrinho.

## Pedidos

Foi desenvolvido uma tentativa de sistema de pedidos de compra com o objetivo de 
integrar o fluxo do carrinho com a finalização da compra. 
O usuário pode possuir apenas um carrinho ativo por vez e, ao finalizá-lo, 
gera um pedido que contém as informações do pedido.

Então quando o pedido é criado vai gerar o estado `CREATED` depois disso ele pode passar
pelo estado de pago (`PAID`), enviado(`SHIPPED`), entregue(`DELIVERED`) ou até mesmo
cancelado(`CANCELLED`).

Esses foram os Endpoints da classe `Pedido`

![endpoints_pedido.png](Images/Endpoints/endpoints_pedido.png)

A estruta da tabalea pode ser melhor visualizada abaixo onde tem o `ID` daquele
pedido, quando foi criado, o `status` dele, o valor total daquele pedido e 
o `USER_ID` que faz a ponte com o carrinho e o pedido.

![tabela_pedidos.png](Images/Tabelas/tabela_pedidos.png)
...
```
Este código acima seria o `PedidoService.java` onde contêm justamente as 
funções de pagar, enviar, entregar e cancelar o pedido.